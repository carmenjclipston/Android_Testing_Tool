...
$adbCommands = @(
    ...
    @{ "Description" = "Get package version number"; "Command" = "`$packageName = Read-Host 'Enter package name'; adb shell dumpsys package `$packageName | findstr versionCode" }
    ...
    @{ "Description" = "List OBB files with size"; "Command" = "`$obbFolder = Read-Host 'Enter OBB folder name'; adb shell du -h /storage/emulated/0/Android/obb/`$obbFolder/*" }
    ...
    @{ "Description" = "Get package name"; "Command" = "(aapt2 dump badging base.apk | Select-String 'package: name=').Line.Split('''')[1]" }
    @{ "Description" = "Get install location"; "Command" = "aapt2 dump badging base.apk | findstr /r /c:`"install-location`"" }
    @{ "Description" = "Check android.hardware.vr.headtracking"; "Command" = "aapt2 dump badging base.apk | findstr /r /c:`"android.hardware.vr.headtracking`"" }
    @{ "Description" = "Check Min and Target SDK versions"; "Command" = "aapt2 dump badging base.apk | findstr /c:`"sdkVersion`" /c:`"targetSdkVersion`"" }
    @{ "Description" = "List all features used"; "Command" = "aapt2 dump badging base.apk | findstr `"uses-feature`"" }
    @{ "Description" = "List all permissions used"; "Command" = "aapt2 dump permissions base.apk" }
    @{ "Description" = "64-bit binary check"; "Command" = "aapt2 dump badging base.apk | findstr `"native-code`"" }
    @{ "Description" = "Debuggable status"; "Command" = "aapt2 dump badging base.apk | findstr /C:""android:debuggable"" /C:""application-debuggable"""}
